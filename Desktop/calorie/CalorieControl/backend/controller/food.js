...
const getCalories=async(req,res)=>{
  try {
    ...
    const dietDoc = await db.collection(userCollection).find({ });

    if (!dietDoc) {
      return res.status(404).send('No diet document found for today');
    }

    const dietArray=await dietDoc.toArray()

    const dietSummaryArray = dietArray.map((diet) => {
      return {
        date: diet.date,          // Extract the date from the diet object
        ...
        // Extract the totalCalories from the diet object
      };
    });
...
const addFood = async (req, res) => {
  try {
    ...
    // Log collection name and update operation
