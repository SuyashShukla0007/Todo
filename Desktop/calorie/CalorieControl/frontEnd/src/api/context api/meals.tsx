...
// type data={
//   date:string,totalCalories:number,totalCarbohydrates:number,totalProtein:number,totalFat:number
// }
...
const [carbohydrates,setCarbohydrates]=useState(0)
  ...
  const getMeals = async () => {
    try {
      const res = await axios.get('/api/meal/foods');
  ...
  const fetchCalories = async () => {
    try {
      const res = await axios.get('/api/meal/calorie');
      const data = res.data;
  console.log(data)
